...
| `MILES_SANDBOX_STATE_ROOT` | `/` | Scan root for the snapshot — governs BOTH phases, so the begin (before) and end (delta) archives cover the same tree. Default `/` scans the whole image; scope it to the agent workspace (for example, `/app` for the Terminal-Bench 2 smoke task) to keep begin-bytes small. Strongly recommended when `MILES_SANDBOX_STATE_BEGIN_BYTES=1` |
