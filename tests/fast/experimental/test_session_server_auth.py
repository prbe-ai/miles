...
_STUBS = {
    ...
    "miles.utils.logging_utils": _install_stub(
        "miles.utils.logging_utils", configure_logger_raw=lambda value: None
    ),
}
...
async def test_session_server_requires_configured_bearer_token() -> None:
    args = SimpleNamespace(miles_router_timeout=30, session_server_api_key="session-secret")
    ...
    async with httpx.AsyncClient(
        ...
        assert (
            await client.get("/health", headers={"Authorization": "Bearer wrong-secret"})
        ).status_code == 401
